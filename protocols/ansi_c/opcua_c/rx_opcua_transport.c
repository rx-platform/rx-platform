...
rx_protocol_result_t opcua_parse_regular_message(opcua_transport_protocol_type* transport, const opcua_transport_header* header, rx_const_packet_buffer* buffer, rx_packet_id_type id)
{
...
rx_protocol_result_t opcua_parse_error_message(opcua_transport_protocol_type* transport, const opcua_transport_header* header, rx_const_packet_buffer* buffer, rx_packet_id_type id)
{
	return RX_PROTOCOL_NOT_IMPLEMENTED;
}

rx_protocol_result_t opcua_parse_pipe_message(opcua_transport_protocol_type* transport, const opcua_transport_header* header, rx_const_packet_buffer* buffer, rx_packet_id_type id)
{
	rx_protocol_result_t result;
	opcua_security_simetric_header* sec_header;
	opcua_sequence_header* sequence_header;
	...
	else if (header->is_final == 'F' && transport->last_chunk_received)
	{
		...
		result = rx_move_packet_up(&transport->protocol_stack_entry, buffer, id);
	}
	else
	{
		...
		{// final chunk
			...
			result = rx_move_packet_up(&transport->protocol_stack_entry, &upward_buffer, id);
			if (result == RX_PROTOCOL_OK)
	...
	while (1)
	{
		...
		if (transport->server_side)
		{
			...
			case opcua_regular_msg_type:
				result = opcua_parse_regular_message(transport, header, buffer, id);
				break;
			case rx_pipe_msg_type:
				result = opcua_parse_pipe_message(transport, header, buffer, id);
				break;
			case opcua_error_msg_type:
				result = opcua_parse_error_message(transport, header, buffer, id);
				break;
			case opcua_hello_msg_type:
				result = opcua_parse_hello_message(transport, header, buffer, id);
				break;
			case opcua_open_msg_type:
				result = opcua_parse_open_message(transport, header, buffer, id);
				break;
			case opcua_close_msg_type:
				result = opcua_parse_close_message(transport, header, buffer, id);
				break;
			default:
				result = RX_PROTOCOL_PARSING_ERROR;
			}
		...
		else
		{
			...
			case opcua_reverse_hello_msg_type:
				result = opcua_parse_reverse_hello_message(transport, header, buffer, id);
				break;
			case opcua_ack_msg_type:
				result = opcua_parse_ack_message(transport, header, buffer, id);
				break;
			case opcua_error_msg_type:
				result = opcua_parse_error_message(transport, header, buffer, id);
				break;
...
{
	rx_protocol_result_t result;
	rx_init_stack_entry(&transport->protocol_stack_entry);
	...
	, rx_protocol_result_t result)
{
	...
	{// send another next packet from queue
		...
		if (result != RX_PROTOCOL_OK)
			return result;
		result = rx_move_packet_down(&transport->protocol_stack_entry, &buffer, 0);//!!!!!!
		if (result != RX_PROTOCOL_OK)
		{
			result = rx_deinit_packet_buffer(&buffer);
			return result;
