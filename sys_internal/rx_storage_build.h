...
class configuration_storage_builder : public rx_platform_builder  
{
  ...
  private:
	  ...
	  template<class T>
	  rx_result create_concrete_type_from_storage(meta_data& meta, rx_storage_item_ptr& storage
          , rx_directory_ptr dir, bool save, tl::type2type<T>);
	  template<class T>
	  rx_result create_concrete_simple_type_from_storage(meta_data& meta, rx_storage_item_ptr& storage
          , rx_directory_ptr dir, bool save, tl::type2type<T>);
	  rx_result create_concrete_relation_type_from_storage(meta_data& meta, rx_storage_item_ptr& storage
          , rx_directory_ptr dir, bool save);
      rx_result create_concrete_data_type_from_storage(meta_data& meta, rx_storage_item_ptr& storage
          , rx_directory_ptr dir, bool save);
	  template<class T>
