...
template <typename allocT, bool swap_bytes>
bool binary_reader<allocT,swap_bytes>::read_data_type (const char* name, data::runtime_data_model& values)
{
    if (name)
    {
        if (!start_object(name))
    ...
    if (!start_array("elements"))
        return false;

    while(!array_end())
    {
        data::runtime_model_element one;

        if (!start_object("Elem"))
            return false;

        if (!read_string("Name", one.name))
            return false;

        if (!read_string("Desc", one.description))
            return false;

        uint8_t type = 0;

        if (!read_byte("Type", type))
            return false;
        ...
        values.elements.push_back(std::move(one));
...
template <typename allocT, bool swap_bytes>
bool binary_writer<allocT,swap_bytes>::write_data_type (const char* name, const data::runtime_data_model& data)
{
    if (name)
    {
        if (!start_object(name))
    ...
    if (!start_array("elements", data.elements.size()))
        return false;

    for (const auto& one : data.elements)
    {
        if (!start_object("Elem"))
            return false;
        ...
        {
            if (!write_string("Name", one.name.c_str()))
                return false;
        }

        if (one.description.empty())
        {
            if (!write_string("Desc", ""))
                return false;
        }
        else
        {
            if (!write_string("Desc", one.description.c_str()))
                return false;
        }

        uint8_t type = (uint8_t)one.value.index();

        if (!write_byte("Type", type))
            return false;
        ...
        switch (type)
        {
            case 0:
                {
                    if (!std::get<rx_simple_value>(one.value).serialize("val", *this))
                        return false;
                }
                break;
            case 1:
