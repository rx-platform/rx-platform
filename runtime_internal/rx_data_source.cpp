...
void data_controler::execute_result_received (value_handle_type handle, rx_result&& result, values::rx_simple_value data, runtime_transaction_id_t id)
{
	if (id == 0)
		return;// nothing to do
	auto it = registered_values_.find(handle);
	if (it != registered_values_.end())
	{
		...
		if (count == 1)
		{// just move the result
			(*it->second.begin())->single_execute_result_received(std::move(result), std::move(data), id);
		}
		else if (count > 0)
		{
			size_t last = count - 1;// it is more readable this way
			...
			{
				if (counter == last)
					done = (*itv)->single_execute_result_received(std::move(result), std::move(data), id);
				else
