...
rx_result_with<runtime_handle_t> binded_tags::bind_tag (const rt_value_ref& ref, runtime_handle_t handle, binded_callback_t callback)
{
	// fill out the data
	switch (ref.ref_type)
	{
...
rx_result_with<runtime_handle_t> binded_tags::bind_item (const string_type& path, runtime_init_context& ctx, binded_callback_t callback)
{
	...
	auto bind_result = bind_tag(ref, handle, callback);
	if (bind_result)
	{
