...
#define RX_STARTUP_HEAP_SIZE (16*1024*1024)
#endif
...
#ifndef RX_STATIC_MEMORY_SIZE
#define RX_STATIC_MEMORY_SIZE (16*1024*1024)
#endif

uint8_t g_heap_memory_buffer[RX_STATIC_MEMORY_SIZE];
uint8_t* g_heap_static_memory = g_heap_memory_buffer;
size_t g_heap_static_memory_size = RX_STATIC_MEMORY_SIZE;
...
{
	if (next_buffer && next_buffer_size >= size && next_buffer != HEAP_NEXT_BUFFER_CLAIMING)
	{
...
size_t rx_init_heap(size_t initial_heap, size_t alloc_size, size_t trigger, size_t bucket_size)
{
	...
	{
		enter_init_spin_lock();
		////////////////////////////////////
		if (current_buffer == NULL)
		{
			rx_slim_lock_create(&heap_lock);
			rx_slim_lock_aquire(&heap_lock);
			current_buffer = g_heap_static_memory;
			current_buffer_size = g_heap_static_memory_size;
			memzero(current_buffer, g_heap_static_memory_size);// claim it in memory
			next_free_index = 0;
			total_heap_bytes = g_heap_static_memory_size;
			used_heap_bytes = 0;
